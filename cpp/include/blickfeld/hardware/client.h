...
std::shared_ptr<grpc::Channel> connect_to_device(const std::string&                        fqdn_or_ip,
    ...
    // create channel credentials for secure TLS communication to TLS server
    auto channel_credentials_options = grpc::SslCredentialsOptions({ .pem_root_certs = base::GRPC_DEFAULT_DEVICE_CA });
    auto channel_credentials         = grpc::SslCredentials(channel_credentials_options);
