...
[NOTE]
====
Events are served in two namespaces: the proprietary `Blickfeld` namespace and, where applicable, its standard `ONVIF` alternative. In such cases the same underlying event is published under both topics — a VMS can subscribe to whichever it supports.
====

[NOTE]
====
Events originating from dynamic event sources (e.g. intrusion event from a particular security zone) are called `Dynamic`. Other events are called `Static`.
====

[NOTE]
====
The https://www.blickfeld.com/lidar-sensor-products/qbprotect/[QbProtect] has to be re-added into VMS in order to re-expose Blickfeld namespace events from dynamic sources newly configured via QbProtect xref:introduction:webgui_tutorial.adoc[WebGUI].
====
...
[%header,cols="1,6,1"]
|===
|Availability |Topic and Description | Namespace
...
----
Intrusion event in the pre-configured security zone, where the name of the security zone should be used as a `Rule`.

*Also served as:* `tns1:Blickfeld/IntrusionZone{Name}`
...
*Also served as:* `tns1:Blickfeld/Tampering`
...
*Also served as:* `tns1:RuleEngine/FieldDetector/ObjectsInside`
...
*Also served as:* `tns1:VideoSource/GlobalSceneChange/AnalyticsService`
|===
