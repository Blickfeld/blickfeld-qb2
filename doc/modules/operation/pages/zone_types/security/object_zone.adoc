...
The object-based security zone uses detected objects and specific object attributes (e.g., size, lifetime, traveled distance, movement direction) within the zone's volume to generate an alarm.
...
Intrusion on wrong direction (default: false):: An alarm is only raised if an object moves in a specific direction. This makes the behavior of the object-based zone similar to the one of the xref:zone_types/security/directional_zone.adoc[directional zone], only covering a larger area. To change the triggering direction, the zone needs to be rotated.
Minimum intruder velocity (default: 0.028, unit: stem:[$\frac{m}{s}$]):: The minimum velocity of the detected object before it can trigger an alarm. The default setting prevents still objects from causing false alarms.
+
[NOTE]
====
The minimum intruder velocity only has an effect if the *intrusion on wrong direction* is *enabled*.

This parameter is currently not exposed in the WebGUI. It can be configured through the xref:protocol:blickfeld/percept_pipeline/config/zone_algorithm.adoc#_blickfeld_percept_pipeline_config_ZoneAlgorithm_ObjectBasedSecurity[zone configuration API].
====

Intrusion on partial entry (default: false):: If enabled, an alarm is raised if any point of the object intrudes the zone, otherwise the center of the object is considered for intrusion. Enabling this option might increase the false alarm rate.
...
include::partial$zone_types_security_tampering.adoc[]
...
An object is considered to be intruding when either the object's centroid (xref:#_intrusion_conditions[intrusion on partial entry] is disabled) or any of the object's points (xref:#_intrusion_conditions[intrusion on partial entry] is enabled) is located within the zone's xref::zone_types/index.adoc#_geometric[cuboid]. In order to narrow down the number of relevant objects, the parameters for xref:#_intrusion_conditions[alarm sizes], xref:#_intrusion_conditions[intrusion on wrong direction], the xref:#_intrusion_conditions[minimum intruder velocity], the xref:#_intrusion_conditions[minimum intruder lifetime], the xref:#_intrusion_conditions[minimum intruder track length] and the xref:#_intrusion_conditions[in-background] property are applied.
