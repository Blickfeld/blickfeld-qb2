...
.2+| *disable_noise_filter* | bool| - | - 
3+| Noise filter for volume measurement. 
 
Disables the noise filter, which performs 1D-clustering and noise removal to prevent spikes 
in the volume measurement due to noise or other disturbances.
...
.2+| *minimum_coverage* | float| - | - 
3+| Minimum coverage of bounding box 
 
A SecurityZoneTampering event is emitted when the coverage is below the specified minimum coverage. 
 
Default: 0 (the coverage estimator is not executed)
...
.2+| *intrusion_on_turnaround* | bool| - | - 
3+| Treat a turnaround inside an area with directional detection as intrusion. 
The parameter only has an effect when intrusion_on_wrong_direction is enabled. 
 
NOTE: This might cause more false alarms as the intrusion condition is more sensitive.
...
Only applicable if intrusion_on_wrong_direction is enabled. 
...
.2+| *disable_rain_filter* | bool| - | - 
3+| Disable the rain filtering as it might aggressively cut vehicles short
