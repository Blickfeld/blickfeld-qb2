...
.2+| *firmware_mismatch* | bool| - | - 
3+| The firmware version does not match the version running on this device. 
A version differing on patch level is allowed. 
 
[NOTE] 
This flag is only set when swarm is enabled.

.2+| *data_loss* | bool| - | - 
3+| Data loss during stream synchronization occurred 
...
3+| Median reflectivity of outlier points

.2+| *adaptive_noise_filter_threshold* | float| - | - 
3+| Reflectivity value used for adaptive noise filtering

.2+| *number_of_adaptive_noise_outliers* | float| - | - 
3+| Number of points that were removed by the adaptive noise filter
