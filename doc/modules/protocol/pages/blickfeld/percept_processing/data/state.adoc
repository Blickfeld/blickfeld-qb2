...
.2+| *coverage* | float| - | - 
3+| The estimated zone coverage

.2+| *coverage_observed_area* | float| - | m^2 
3+| Estimated area of the zone which is observed at all, i.e. the extent `coverage` is measured over. 
 
Zero means the zone is not observed. Deliberately an area and not a beam count: the number of 
beams reaching a zone falls with the square of its distance, so a distant sensor with a complete 
view of the zone would report a far smaller count than a close one which merely clips a corner 
of it. Across a swarm this expresses how well a sensor is placed for the zone, and only the 
well-placed sensors decide whether it is tampered.
...
.2+| *tampering* | xref:blickfeld/percept_processing/data/flag.adoc[Flag] | - | - 
3+| Active tampering of zone. The minimum coverage is too low.
