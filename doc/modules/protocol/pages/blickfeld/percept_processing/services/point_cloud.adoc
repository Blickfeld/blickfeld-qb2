...
3+| If set, point cloud is cropped to given shape. Combined with `crops` (if also set) as a union.

.2+| *crops* | repeated xref:blickfeld/base/geometry/shape.adoc[base.geometry.Shape] | - | - 
3+| If set, the point cloud is additionally cropped to the union of the given shapes. Combined with 
`crop` (if also set) as a union. Useful to reduce payload size when only specific volumes are of 
interest.
